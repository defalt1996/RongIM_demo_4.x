...
public class FilePreviewActivity extends RongBaseActivity implements View.OnClickListener {
    ...
    private void initData() {
        ...
        mFileTypeImage.setImageResource(FileTypeUtils.fileTypeImageId(getApplicationContext(), mFileName));
            ...
            switch (mFileDownloadInfo.state) {
                ...
                case DOWNLOADING:
                    if (supportResumeTransfer == SupportResumeStatus.SUPPORT) {
                        ...
                        RongIM.getInstance().pauseDownloadMediaMessage(mMessage, null);
                        info = getFileInfo();
                        if (info != null) {
                        ...
                        mFileSizeView.setText(getString(R.string.rc_ac_file_download_progress_pause, FileTypeUtils.formatFileSize(downloadedFileLength), FileTypeUtils.formatFileSize(mFileSize)));
    ...
    protected void resetMediaMessageLocalPath() {
        ...
        if (fileMessage instanceof FileMessage) {
    ...
    public void openFile(String fileName, Uri fileSavePath) {
        try {
            if (!openInsidePreview(fileName, fileSavePath)) {
                Intent intent = FileTypeUtils.getOpenFileIntent(getApplicationContext(),fileName, fileSavePath);
    ...
    private void processTxtFile(String fileName, Uri uri) {
        ...
        if (FileUtils.uriStartWithContent(uri)) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                Uri txtUri = FileProvider.getUriForFile(getApplicationContext(),
                        getApplicationContext().getPackageName()
                                + getApplicationContext().getResources().getString(R.string.rc_authorities_fileprovider), new File(path));
    ...
    private void downloadFile() {
        ...
        if (supportResumeTransfer == SupportResumeStatus.SUPPORT) {
            ...
            mFileSizeView.setText(getString(R.string.rc_ac_file_download_progress_tv, FileTypeUtils.formatFileSize(downloadedFileLength), FileTypeUtils.formatFileSize(mFileSize)));
        } else {
            mFileButton.setVisibility(View.GONE);
            mDownloadProgressView.setVisibility(View.VISIBLE);
