...
public class CombineMessageUtils {
    ...
    Uri getUrlFromMessageList(List<Message> messagesList) {
        ...
        String filePath = FileUtils.getCachePath(RongIM.getInstance().getApplicationContext())
    ...
    private String getHtmlFromMessageContent(Message message, MessageContent content) {
        ...
        if (context == null
                || tag == null || !tag.value().startsWith("RC:")) {
        ...
        switch (type) {
            ...
            case TAG_FILE: // 文件
                ...
                html = html.replace(MSG_FILE_NAME, file.getName())
                        ...
                        .replace(MSG_FILE_ICON, getBase64FromImageId(FileTypeUtils.fileTypeImageId(context, file.getName())));
                break;
            case TAG_LBS: // 位置
            ...
            case TAG_COMBINE: // 合并
                ...
                html = html.replace(MSG_FILE_URL, combine.getMediaUrl() == null ? "" : combine.getMediaUrl().toString())
                        ...
                        .replace(MSG_FOOT, context.getString(R.string.rc_combine_chat_history));
                break;
            default:
    ...
    private String getJson() {
        ...
        try {
            bf = new BufferedReader(new InputStreamReader(
                    RongIM.getInstance().getApplicationContext().getAssets().open(JSON_FILE_NAME)));
            String line;
    ...
    private String getBase64FromUrl(Uri uri) {
        ...
        Bitmap bitmap = null;
        try {
            bitmap = BitmapUtil.getResizedBitmap(RongIM.getInstance().getApplicationContext(), uri, IMAGE_WIDTH, IMAGE_HEIGHT);
        } catch (IOException e) {
    ...
    private String getBase64FromImageId(int id) {
        ...
        Bitmap bitmap = BitmapFactory.decodeResource(RongIM.getInstance().getApplicationContext().getResources(), id);
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        bitmap.compress(Bitmap.CompressFormat.PNG, 100, baos);
        byte[] data = baos.toByteArray();
        return BASE64_PRE + Base64.encodeToString(data, Base64.NO_WRAP);
    ...
    private String getSpannable(MessageContent content) {
        Spannable spannable = RongContext.getInstance().getMessageTemplate(content.getClass())
                .getContentSummary(RongIM.getInstance().getApplicationContext(), content);
    ...
    public String getCombineFilePath(String uri) {
        return FileUtils.getCachePath(RongIM.getInstance().getApplicationContext())
                + File.separator + COMBINE_FILE_PATH
