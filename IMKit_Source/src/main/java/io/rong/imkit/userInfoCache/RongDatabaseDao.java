...
class RongDatabaseDao {
    ...
    void open(Context context, String appKey, String currentUserId) {
        ...
        try {
            ...
            db = rongUserCacheDatabaseHelper.getWritableDatabase();
    ...
    UserInfo getUserInfo(final String userId) {
        ...
        UserInfo info = null;
        try {
            c = db.query(usersTable, null, "id = ?", new String[]{userId}, null, null, null);
    ...
    synchronized void putUserInfo(UserInfo userInfo) {
        ...
        try {
            db.execSQL("replace into users (id, name, portrait, extra) values (?, ?, ?, ?)", new String[]{userInfo.getUserId(), userInfo.getName(), userInfo.getPortraitUri() + "", userInfo.getExtra()});
    ...
    GroupUserInfo getGroupUserInfo(final String groupId, final String userId) {
        ...
        try {
            c = db.query(groupUsersTable, null, "group_id = ? and user_id = ?", new String[]{groupId, userId}, null, null, null);
            if (c != null && c.moveToFirst()) {
    ...
    Group getGroupInfo(final String groupId) {
        ...
        Group group = null;
        try {
            c = db.query(groupsTable, null, "id = ?", new String[]{groupId}, null, null, null);
            if (c != null && c.moveToFirst()) {
                String id = c.getString(c.getColumnIndex("id"));
                String name = c.getString(c.getColumnIndex("name"));
                String portrait = c.getString(c.getColumnIndex("portrait"));
    ...
    synchronized void putGroupInfo(Group group) {
        ...
        db.execSQL("replace into groups (id, name, portrait) values (?, ?, ?)", new String[]{group.getId(), group.getName(), group.getPortraitUri() == null ? "" : group.getPortraitUri().toString()});
    }
    ...
    Discussion getDiscussionInfo(final String discussionId) {
        ...
        try {
            c = db.query(discussionsTable, null, "id = ?", new String[]{discussionId}, null, null, null);
            if (c != null && c.moveToFirst()) {
                String id = c.getString(c.getColumnIndex("id"));
                String name = c.getString(c.getColumnIndex("name"));
                //String portrait = c.getString(c.getColumnIndex("portrait"));
