...
public class HQVoiceMessageItemProvider extends IContainerItemProvider.MessageProvider<HQVoiceMessage> {
    private final static String TAG = "HQVoiceMessageItemProvider";
    ...
    public View newView(Context context, ViewGroup group) {
        View view = LayoutInflater.from(context).inflate(R.layout.rc_item_hq_voice_message, null);
        ...
        holder.downloadError = view.findViewById(R.id.rc_voice_download_error);
        holder.downloadProcessing = view.findViewById(R.id.rc_download_progress);
    ...
    @Override
    public void bindView(final View v, int position, final HQVoiceMessage content, final UIMessage message) {
        final HQVoiceMessageItemProvider.ViewHolder holder = (HQVoiceMessageItemProvider.ViewHolder) v.getTag();
        ...
        if (content.isDestruct()) {
            if (message.getMessageDirection() == Message.MessageDirection.SEND) {
                ...
                DestructManager.getInstance().addListener(message.getUId(), new HQVoiceMessageItemProvider.DestructListener(holder, message), TAG);
    ...
    @Override
    public void onItemClick(final View view, int position, final HQVoiceMessage content, final UIMessage message) {
        if (content == null) return;
        RLog.d(TAG, "Item index:" + position + " content.getLocalPath():" + content.getLocalPath());
        final HQVoiceMessageItemProvider.ViewHolder holder = (HQVoiceMessageItemProvider.ViewHolder) view.getTag();
        ...
        final boolean listened = message.getMessage().getReceivedStatus().isListened();
        playOrDownloadHQVoiceMsg(view, content, message, holder, listened);
    }

    private void playOrDownloadHQVoiceMsg(final View view, final HQVoiceMessage content, UIMessage message, final ViewHolder holder, final boolean listened) {
        boolean ifDownloadHQVoiceMsg = (content.getLocalPath() == null || TextUtils.isEmpty(content.getLocalPath().toString()));
        if (message.getMessageDirection() == Message.MessageDirection.RECEIVE) {
            ifDownloadHQVoiceMsg = (content.getLocalPath() == null || TextUtils.isEmpty(content.getLocalPath().toString()) || !isFileExists(content.getLocalPath().toString()));
        }
        if (ifDownloadHQVoiceMsg) {
            downloadHQVoiceMsg(view, message, holder, listened);
        } else {
            playHQVoiceMessage(view, content, message, holder, listened);
        }
    }
    ...
    private void downloadHQVoiceMsg(final View view, final UIMessage uiMessage, final ViewHolder holder, final boolean listened) {
        RongIM.getInstance().downloadMediaMessage(uiMessage.getMessage(), new IRongCallback.IDownloadMediaMessageCallback() {
            ...
            public void onSuccess(Message message) {
                RLog.d(TAG, "playOrDownloadHQVoiceMsg onSuccess");
                holder.downloadError.setVisibility(View.GONE);
                holder.downloadProcessing.setVisibility(View.GONE);
                playHQVoiceMessage(view, ((HQVoiceMessage) message.getContent()), uiMessage, holder, listened);
            }
            ...
            public void onError(Message message, RongIMClient.ErrorCode code) {
                RLog.d(TAG, "playOrDownloadHQVoiceMsg onError");
    ...
    private void playHQVoiceMessage(View view, HQVoiceMessage content, UIMessage message, ViewHolder holder, boolean listened) {
        ...
        AudioPlayManager.getInstance().startPlay(view.getContext(), content.getLocalPath(), new
    ...
    private void setLayout(Context context, HQVoiceMessageItemProvider.ViewHolder holder, UIMessage message, boolean playing) {
        ...
        if (message.getMessageDirection() == Message.MessageDirection.SEND) {
            ...
            RLog.d(TAG, "message.getExtra() = " + message.getExtra());
            holder.downloadProcessing.setVisibility(View.VISIBLE);
            holder.downloadError.setVisibility(View.GONE);
            ...
            if (!message.getReceivedStatus().isListened()
                    && ((HQVoiceMessage) message.getContent()).getLocalPath() != null)
    ...
    private class VoiceMessagePlayListener implements IAudioPlayListener {
        ...
        public void onStart(Uri uri) {
            ...
            if (message.getContent().isDestruct() && message.getMessageDirection().equals(Message.MessageDirection.RECEIVE)) {
                ...
                EventBus.getDefault().post(new Event.changeDestructionReadTimeEvent(message.getMessage()));
            }
        }
        ...
        public void onComplete(Uri uri) {
            ...
            if (message.isListening() && message.getMessageDirection().equals(Message.MessageDirection.RECEIVE)) {
                try {
                    ...
                    RLog.e(TAG, "VoiceMessagePlayListener.onComplete", e);
                }
            }
    ...
    private static class DestructListener implements RongIMClient.DestructCountDownTimerListener {
        ...
        public void onTick(long millisUntilFinished, String messageId) {
            if (mUIMessage.getUId().equals(messageId)) {
                HQVoiceMessageItemProvider.ViewHolder viewHolder = mHolder.get();
