...
public class DiscussionNotificationMessageItemProvider extends IContainerItemProvider.MessageProvider<DiscussionNotificationMessage> {
    ...
    public void bindView(View v, int position, DiscussionNotificationMessage content, UIMessage message) {
        ...
        Spannable spannable = getContentSummary(v.getContext(), content);
    ...
    public Spannable getContentSummary(Context context, DiscussionNotificationMessage data) {
        ...
        return new SpannableString(getWrapContent(context, data));
    }
     ...
     * @return 拼完后可直接显示的消息。
     */
    private String getWrapContent(Context context, DiscussionNotificationMessage discussionNotificationMessage) {
            ...
            case DISCUSSION_MEMBER_INVITE:
                ...
                if (currentUserId.equals(operatorId)) {
                    if ("1".equals(extension)) {//关闭
                        content = context.getResources().getString(R.string.rc_discussion_nt_msg_for_you_is_open_invite_close);
                    } else if ("0".equals(extension)) {//打开
                        content = context.getResources().getString(R.string.rc_discussion_nt_msg_for_you_is_open_invite_open);
                    }
                } else {//开放
                    UserInfo operatorUserInfo = RongUserInfoManager.getInstance().getUserInfo(operatorId);
